...
void Scene::Update(Camera& cam, GameState& gameState, InputState& inputState)
{
                                             ...
                                             sf::IntRect& viewbounds)
{
    std::function<void(QuadTree*)> populateGhostGrid = [&](QuadTree* node)
    {
        ...
        int sizeX = node->quadRect.getSize().x * cam.Scale.x;
        int sizeY = static_cast<float>(node->quadRect.getSize().y) / 2 * cam.Scale.y +
                    static_cast<float>(tileSize) / 2 * quadTreeDepth * cam.Scale.y;
...
void Scene::findViewportIterators(QuadTree* root, Camera& cam, GridGenerator& gridGenerator, sf::IntRect& viewbounds)
{
    ...
    int sizeX = root->quadRect.getSize().x * cam.Scale.x;
    int sizeY = static_cast<float>(root->quadRect.getSize().y) / 2 * cam.Scale.y +
                static_cast<float>(TILE_SIZE) / 2 * MAX_TILE_DEPTH * cam.Scale.y;
    ...
    for (auto iter = gameScene.begin(); iter != gameScene.end(); iter++)
    {
        ...
        if (currentCell.Objects.size() != 0)
        {
            for (int i = 0; i < currentCell.Objects.size(); i++)
            {
				...
				if (objectSprite.getTexture() == nullptr)
                    continue;
