											 ...
											 sf::IntRect& viewbounds)
{
	std::function<void(QuadTree*)> populateGhostGrid = [&](QuadTree* node)
	{
		int screenX, screenY;
		auto v = sf::Vector2f();
		v.x = static_cast<float>(node->quadRect.getPosition().x) / TILE_SIZE;
		v.y = static_cast<float>(node->quadRect.getPosition().y) / TILE_SIZE;
		sf::Vector2f isometricPosition = gridGenerator.cartesianToIsometricTransform(v);
