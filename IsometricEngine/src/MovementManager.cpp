...
void MovementManager::SetUnitPath(PathfinderAgent* agent, BattlefieldCell* goal, GameStateManager* gameStateManager,
								  InputState& state, Scene& scene, Camera* camera)
{
	if (goal != nullptr)
	{
		...
		BattlefieldCell* startCell = gameStateManager->getState().quadTree->getCell(
			gameStateManager->state.quadTree, agent->getPosXIndex() * 100, agent->getPosYIndex() * 100, 4);

		GridGenerator generator;
		sf::IntRect viewbounds(0, 0, camera->window.getSize().x, camera->window.getSize().y);
		ghostGrid =
			scene.generateGhostGridFromScene(gameStateManager->getState().quadTree, *camera, generator, viewbounds);
...
void MovementManager::ExploreNeighbours(BattlefieldCell* current, BattlefieldCell* goal)
{
	...
	for (int i = 0; i < 8; ++i)
	{
		...
		int levelInt = GlobalConstants::quadTreeDepth;
